...
let moviePage = 1, kdramaPage = 1, animePage = 1;
let movieGenre = '', kdramaGenre = '', animeGenre = '';
...
const AUDIO_TRACKS = [
  { code: 'original', flag: '🔊', label: 'Original Audio',   desc: 'Native language of the content' },
  { code: 'en',       flag: '🇺🇸', label: 'English (DUB)',   desc: 'English dubbed version' },
  { code: 'ja',       flag: '🇯🇵', label: 'Japanese (DUB)',  desc: 'Japanese dubbed / original anime' },
  { code: 'ko',       flag: '🇰🇷', label: 'Korean (DUB)',    desc: 'Korean dubbed / original K-Drama' },
  { code: 'zh',       flag: '🇨🇳', label: 'Chinese (DUB)',   desc: 'Mandarin dubbed version' },
  { code: 'es',       flag: '🇪🇸', label: 'Spanish (DUB)',   desc: 'Spanish dubbed version' },
  { code: 'fr',       flag: '🇫🇷', label: 'French (DUB)',    desc: 'French dubbed version' },
  { code: 'de',       flag: '🇩🇪', label: 'German (DUB)',    desc: 'German dubbed version' },
  { code: 'pt',       flag: '🇧🇷', label: 'Portuguese (DUB)',desc: 'Portuguese dubbed version' },
  { code: 'hi',       flag: '🇮🇳', label: 'Hindi (DUB)',     desc: 'Hindi dubbed version' },
  { code: 'ar',       flag: '🇸🇦', label: 'Arabic (DUB)',    desc: 'Arabic dubbed version' },
];
...
function checkApiKey() {
  if (TMDB_KEY === 'YOUR_TMDB_API_KEY_HERE') {
    ...
    banner.id = 'apiBanner';
    banner.style.cssText = 'position:fixed;top:64px;left:0;right:0;z-index:300;background:#e8b44b;color:#000;text-align:center;padding:10px 16px;font-weight:600;font-size:13px;';
    banner.innerHTML = '⚠️ No TMDB API key! Open app.js and replace YOUR_TMDB_API_KEY_HERE with your free key from <a href="https://www.themoviedb.org/settings/api" target="_blank" style="text-decoration:underline">themoviedb.org</a>. Demo mode active.';
...
function switchTab(tab) {
  ...
  document.getElementById('heroSection').style.display = tab === 'home' ? '' : 'none';
  const cont = getContinue();
  document.getElementById('continueSection').style.display = (tab === 'home' && cont.length) ? '' : 'none';
  window.scrollTo({ top: 0, behavior: 'smooth' });

  if (tab === 'movies' && !document.getElementById('movieGrid').children.length) loadMoviesPage();
  if (tab === 'kdrama' && !document.getElementById('kdramaGrid').children.length) loadKdramaPage();
  if (tab === 'anime' && !document.getElementById('animeGrid').children.length) loadAnimePage();
...
async function loadHome() {
  ...
  try {
    const [trending, kdramas, anime, newRel] = await Promise.all([
      ...
      tmdb('/discover/tv', { with_origin_country:'KR', sort_by:'popularity.desc', language:'en-US' }),
...
function showHeroSlide(i) {
  ...
  wBtn.onclick = () => { toggleWatchlist(item, type); showHeroSlide(heroIndex); };
  updateHeroDots();
}
...
function createCard(item, type) {
  ...
  const poster = item.poster_path ? `${IMG}w300${item.poster_path}` : 'https://via.placeholder.com/155x232/0f1520/6b7a8d?text=No+Image';
...
function createGridCard(item, type) {
  ...
  const title = item.title || item.name || 'Unknown';
  const poster = item.poster_path ? `${IMG}w300${item.poster_path}` : 'https://via.placeholder.com/160x240/0f1520/6b7a8d?text=No+Image';
  const rating = (item.vote_average||0).toFixed(1);
  div.innerHTML = `
...
const MOVIE_GENRES = [
  ...
  {id:'878', name:'Sci-Fi'},{id:'53', name:'Thriller'},{id:'16', name:'Animation'},
  {id:'12', name:'Adventure'},{id:'80', name:'Crime'},
];

async function loadMoviesPage() {
  buildGenreFilters('movieGenres', MOVIE_GENRES, (id) => {
    movieGenre = id; moviePage = 1;
    document.getElementById('movieGrid').innerHTML = '';
    loadMoviesPage();
  });
  loadMoreMovies(true);
  document.getElementById('loadMoreMovies').onclick = () => loadMoreMovies(false);
}

async function loadMoreMovies(reset) {
  if (reset) { moviePage = 1; document.getElementById('movieGrid').innerHTML = ''; }
  try {
    ...
    moviePage++;
  } catch(e) { showToast('Error loading movies'); }
}

// ─── KDRAMA PAGE ──────────────────────────────
const KDRAMA_GENRES = [
  {id:'', name:'All'},{id:'18', name:'Drama'},{id:'10749', name:'Romance'},
  {id:'35', name:'Comedy'},{id:'80', name:'Crime'},{id:'9648', name:'Mystery'},
  {id:'10765', name:'Fantasy'},{id:'10759', name:'Action'},
];

async function loadKdramaPage() {
  buildGenreFilters('kdramaGenres', KDRAMA_GENRES, (id) => {
    kdramaGenre = id; kdramaPage = 1;
    document.getElementById('kdramaGrid').innerHTML = '';
    loadKdramaPage();
  });
  loadMoreKdrama(true);
  document.getElementById('loadMoreKdrama').onclick = () => loadMoreKdrama(false);
}

async function loadMoreKdrama(reset) {
  if (reset) { kdramaPage = 1; document.getElementById('kdramaGrid').innerHTML = ''; }
  try {
    const params = { with_origin_country:'KR', sort_by:'popularity.desc', page: kdramaPage, language:'en-US' };
    ...
    kdramaPage++;
  } catch(e) { showToast('Error loading K-Dramas'); }
}

// ─── ANIME PAGE ───────────────────────────────
const ANIME_GENRES = [
  {id:'', name:'All'},{id:'16', name:'Animation'},{id:'28', name:'Action'},
  {id:'12', name:'Adventure'},{id:'14', name:'Fantasy'},{id:'878', name:'Sci-Fi'},
  {id:'35', name:'Comedy'},{id:'10765', name:'Supernatural'},
];

async function loadAnimePage() {
  buildGenreFilters('animeGenres', ANIME_GENRES, (id) => {
    animeGenre = id; animePage = 1;
...
async function loadMoreAnime(reset) {
  ...
  try {
    const params = { with_genres:'16', with_origin_country:'JP', sort_by:'popularity.desc', page: animePage };
    if (animeGenre && animeGenre !== '16') params.with_genres = '16,' + animeGenre;
    const data = await tmdb('/discover/tv', params);
    appendGridCards('animeGrid', data.results, 'tv');
    animePage++;
  } catch(e) { showToast('Error loading Anime'); }
}
...
function buildGenreFilters(containerId, genres, onSelect) {
  ...
  genres.forEach(g => {
    ...
    btn.addEventListener('click', () => {
      c.querySelectorAll('.genre-btn').forEach(b => b.classList.remove('active'));
...
async function doSearch(query) {
  if (TMDB_KEY === 'YOUR_TMDB_API_KEY_HERE') return;
  const results = document.getElementById('searchResults');
  results.innerHTML = '<div style="padding:16px;color:var(--text-muted);font-size:13px;">Searching...</div>';
  ...
  try {
    ...
    if (!items.length) {
      results.innerHTML = '<div style="padding:16px;color:var(--text-muted);font-size:13px;">No results found</div>';
      return;
    }
    results.innerHTML = items.map(item => {
      const title = item.title || item.name || '';
      const poster = item.poster_path ? `${IMG}w92${item.poster_path}` : 'https://via.placeholder.com/40x56/0f1520/6b7a8d?text=?';
    ...
    results.innerHTML = '<div style="padding:16px;color:var(--text-muted);font-size:13px;">Search failed</div>';
  }
}
...
async function openDetail(item, type) {
  ...
  document.getElementById('detailBg').style.backgroundImage = backdrop ? `url(${backdrop})` : '';
  document.getElementById('detailPoster').src = poster;
  document.getElementById('detailTitle').textContent = title;
  document.getElementById('detailBadge').textContent = type === 'movie' ? '🎬 MOVIE' : type === 'tv' ? '📺 SERIES' : '⚡ ANIME';
  ...
  document.getElementById('detailPlay').onclick = () => { closeDetail(); openPlayer(item, type); };
  wBtn.onclick = () => {
  ...
  if (TMDB_KEY !== 'YOUR_TMDB_API_KEY_HERE') {
    try {
      ...
      const runtime = type === 'movie' ? (details.runtime ? `${details.runtime}m` : '') : (details.number_of_seasons ? `${details.number_of_seasons} Season${details.number_of_seasons>1?'s':''}` : '');
      document.getElementById('detailMeta').innerHTML =
        `<span class="detail-rating">★ ${rating}</span>
         <span>${year}</span>
...
function setupPlayerModal() {
  ...
  document.getElementById('loadEpBtn').addEventListener('click', () => {
    ...
    loadPlayerFrame();
    saveContinue(playerCurrentItem, playerCurrentType, playerSeason, playerEpisode);
  });
  document.getElementById('seasonSelect').addEventListener('change', populateEpisodes);
}
...
async function openPlayer(item, type) {
  ...
  if (type === 'tv') {
    ...
    if (cont) {
      ...
      playerSeason = 1; playerEpisode = 1;
    }
    document.getElementById('modalSub').textContent = `S${playerSeason} E${playerEpisode}`;
  } else {
...
function loadPlayerFrame() {
  ...
  const s = playerSeason, ep = playerEpisode;
  ...
  document.getElementById('modalSub').textContent = type === 'tv' ? `S${s} E${ep}` : '';
...
function buildSourceButtons() {
  ...
  SOURCES.forEach(src => {
    ...
    const qualityClass = src.quality === '4K' ? 'q-4k' : src.quality === '1080p' ? 'q-fhd' : 'q-hd';
...
function buildAudioControls(nativeLang) {
  ...
  const tracksToShow = [
    { code: 'original', flag: nativeFlag,  label: `Original (${nativeLang.toUpperCase()})`, desc: `Native ${nativeName} audio` },
  ];

  // Add English dub if content is not originally English
  if (nativeLang !== 'en') {
    tracksToShow.push({ code: 'en', flag: '🇺🇸', label: 'English (DUB)', desc: 'English dubbed audio' });
  }

  // Add other common dubs
  const extras = [
    { code:'es', flag:'🇪🇸', label:'Spanish (DUB)', desc:'Spanish dubbed audio' },
    { code:'fr', flag:'🇫🇷', label:'French (DUB)',   desc:'French dubbed audio' },
    { code:'de', flag:'🇩🇪', label:'German (DUB)',   desc:'German dubbed audio' },
    { code:'pt', flag:'🇧🇷', label:'Portuguese (DUB)', desc:'Portuguese dubbed audio' },
    { code:'hi', flag:'🇮🇳', label:'Hindi (DUB)',    desc:'Hindi dubbed audio' },
    { code:'ar', flag:'🇸🇦', label:'Arabic (DUB)',   desc:'Arabic dubbed audio' },
    { code:'zh', flag:'🇨🇳', label:'Chinese (DUB)',  desc:'Chinese dubbed audio' },
  ];
  ...
  tracksToShow.forEach(track => {
    const btn = document.createElement('button');
    btn.className = 'audio-btn' + (currentAudio === track.code ? ' active' : '');
    ...
    btn.addEventListener('click', () => {
      ...
      btnGroup.querySelectorAll('.audio-btn').forEach(b => b.classList.remove('active'));
      btn.classList.add('active');
      updateAudioDisplay();
      loadPlayerFrame();
      const displayName = track.code === 'original'
...
function getWatchlist() {
  try { return JSON.parse(localStorage.getItem('zrntv_watchlist') || '[]'); }
  catch(e) { return []; }
}
...
function renderWatchlist() {
  ...
  list.forEach(item => grid.appendChild(createGridCard(item, item._type || 'movie')));
}

// ─── CONTINUE WATCHING ───────────────────────
function getContinue() {
  try { return JSON.parse(localStorage.getItem('zrntv_continue') || '[]'); }
  catch(e) { return []; }
}
...
function renderContinueWatching() {
  ...
  list.slice(0, 10).forEach(item => {
    ...
    const title = item.title || item.name || '';
    const poster = item.poster_path ? `${IMG}w300${item.poster_path}` : 'https://via.placeholder.com/155x232/0f1520/6b7a8d?text=No+Image';
    const sub = type === 'tv' ? `S${item.season||1} E${item.episode||1}` : 'Movie';
    div.innerHTML = `
      <img class="card-thumb" src="${poster}" alt="${title}" loading="lazy"/>
      <div class="card-overlay">
        <div class="card-play">▶</div>
...
function loadDemoMode() {
  const demoItems = [
    { id:1, title:'Demo Movie 1', vote_average:8.5, release_date:'2024-01-01', overview:'This is a demo. Add your TMDB API key to see real content!', poster_path:null, backdrop_path:null },
    { id:2, title:'Demo Movie 2', vote_average:7.2, release_date:'2024-03-15', overview:'Get your free API key at themoviedb.org to unlock thousands of movies, K-dramas, and anime.', poster_path:null, backdrop_path:null },
    { id:3, title:'Demo Movie 3', vote_average:9.0, release_date:'2023-11-20', overview:'Zrn TV is ready! Just add your API key.', poster_path:null, backdrop_path:null },
  ];

  heroItems = demoItems;
  document.getElementById('heroTitle').textContent = 'Welcome to Zrn TV';
  document.getElementById('heroDesc').textContent = 'Add your free TMDB API key in app.js to start streaming movies, K-Dramas, and Anime!';
  document.getElementById('heroBadge').textContent = '🎬 DEMO MODE';
  document.getElementById('heroMeta').innerHTML = '<span style="color:var(--accent)">Get your API key at themoviedb.org</span>';
  buildHeroDots();
  ...
  ['trendingMovies','popularKdramas','topAnime','newReleases','movieGrid','kdramaGrid','animeGrid'].forEach(id => {
